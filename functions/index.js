...
const ADMIN_EMAILS=new Set(['adewoleadetayom@gmail.com','arowologoodluck2@gmail.com']);
...
exports.exchangeIdToken=onRequest(async(req,res)=>{
  ...
  try{
    ...
    if(decoded.firebase?.sign_in_provider!=='google.com' || !decoded.email || decoded.email_verified!==true)
      return res.status(403).json({error:'A verified Google account is required'});
    const user=await getAuth().getUser(decoded.uid);
    const admin=ADMIN_EMAILS.has((user.email||decoded.email||'').toLowerCase());
    const claims={...(user.customClaims||{}),google_verified:true};
...
exports.ensureAdminClaim=onRequest(async(req,res)=>{
  ...
  try{
    ...
    const user=await getAuth().getUser(decoded.uid);
    if(decoded.firebase?.sign_in_provider!=='google.com' || !user.email || !ADMIN_EMAILS.has(user.email.toLowerCase()))
      return res.status(403).json({error:'Administrator Google account not authorized'});
    await getAuth().setCustomUserClaims(decoded.uid,{...(user.customClaims||{}),google_verified:true,admin:true});
