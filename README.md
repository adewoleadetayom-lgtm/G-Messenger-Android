# G Messenger v23 — combined build

G Messenger v23 combines the v21/v22 account-isolation and UI/function fixes with the three major production-direction upgrades requested for v23:
...
The v23 sample uses anonymous Firebase Authentication plus the app's existing local verification flow. Before a public launch, replace the prototype verification with a real authenticated registration/OTP flow and review the database rules with a security professional.
...
The project is version **23.0.0** / versionCode **23**. GitHub Actions builds the debug APK.

The v23 implementation is intentionally honest about external configuration: Firebase credentials and a Gemma model bundle are required for those production-direction features to operate across devices.
