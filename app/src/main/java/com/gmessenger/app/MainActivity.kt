...
class MainActivity : Activity() {
        ...
        try {
            if (FirebaseApp.getApps(this).isEmpty()) {
                FirebaseApp.initializeApp(this, FirebaseOptions.Builder()
                    .setApiKey("AIzaSyBUVfHX_bW3Pd-gKeYj9zMEsvREe3BMyuk")
                    .setApplicationId("1:812903668410:android:ec1286a83ea6a82b720083")
    ...
    inner class Bridge {
        ...
        @JavascriptInterface fun version() = "26.0.0"
        @JavascriptInterface fun getPushToken(): String = pushToken ?: ""

        @JavascriptInterface fun startPhoneVerification(rawPhone: String) {
            val phone = normalizePhoneE164(rawPhone)
            if (phone == null) {
                web.evaluateJavascript("window.onPhoneAuthError && window.onPhoneAuthError(${JSONObject.quote("Enter a valid Nigerian phone number, e.g. 09033229734")})", null)
                return
            }
            val auth = phoneAuth ?: FirebaseAuth.getInstance().also { phoneAuth = it }
        ...
        @JavascriptInterface fun verifyPhoneCode(code: String) {
            val id = phoneVerificationId
            if (id.isNullOrBlank()) {
                web.evaluateJavascript("window.onPhoneAuthError && window.onPhoneAuthError(${JSONObject.quote("Request a verification code first")})", null)
                return
            }
            val clean = code.trim()
            if (clean.length != 6) {
                web.evaluateJavascript("window.onPhoneAuthError && window.onPhoneAuthError(${JSONObject.quote("Enter the 6-digit SMS code")})", null)
                return
            }
            signInWithPhoneCredential(PhoneAuthProvider.getCredential(id, clean))
        }
        ...
        private fun signInWithPhoneCredential(credential: com.google.firebase.auth.PhoneAuthCredential) {
            ...
            auth.signInWithCredential(credential).addOnCompleteListener { task ->
                if (!task.isSuccessful) {
                    val msg = task.exception?.message ?: "The verification code is invalid or expired"
                    runOnUiThread { web.evaluateJavascript("window.onPhoneAuthError && window.onPhoneAuthError(${JSONObject.quote(msg)})", null) }
                ...
                user?.getIdToken(true)?.addOnSuccessListener { result ->
                    ...
                    runOnUiThread {
                        web.evaluateJavascript("window.onPhoneAuthSuccess && window.onPhoneAuthSuccess(${JSONObject.quote(token)},${JSONObject.quote(phone)},${JSONObject.quote(user.uid)})", null)
                    }
                }?.addOnFailureListener { e ->
                    runOnUiThread { web.evaluateJavascript("window.onPhoneAuthError && window.onPhoneAuthError(${JSONObject.quote(e.message ?: "Could not obtain secure authentication token")})", null) }
                }
            }
        }

        private fun normalizePhoneE164(raw: String): String? {
            val digits = raw.filter { it.isDigit() }
            val normalized = when {
                digits.startsWith("0") && digits.length == 11 -> "+234" + digits.substring(1)
                digits.startsWith("234") && digits.length == 13 -> "+" + digits
                raw.trim().startsWith("+") && digits.length in 10..15 -> "+" + digits
                else -> return null
            }
            return if (normalized.matches(Regex("\\+234[789][01]\\d{8}"))) normalized else null
        }
