(function(){
  ...
  function patch(){
    ...
    window.sendMsg=function(i){
      ...
      if(chat&&chat.messages.length>before){const m=chat.messages[chat.messages.length-1];m.clientId=m.clientId||('c_'+Date.now()+'_'+Math.random().toString(36).slice(2,8));window.save&&window.save();if(chat.global===true)sendGeneralMessage(m).catch(e=>console.warn(e));else sendMessage(chat,m).catch(e=>console.warn(e));}
      return r;
    };
